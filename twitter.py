...
###########################################################################
#  Methods added for the EE364 Project:
#    - UpdateProfile(location=location, description=description, name=name)
#	  Updates the appropriate tags on the authenticated users account
#    - GetFavorites()
#	  Returns the favorites of the authenticated user
#    - UpdateProfile<type>Color(color=color)
#	  Type can be: Background, Sidebar, and Text
#	  Updates the colors associated with the authenticated users profile
#    - GetSentDirectMessages()
#	  Returns all the authenicated user's sent direct messages
############################################################################
...
class Status(object):
  ...
  @staticmethod
  def NewFromJsonDict(data, provided_user=None):
...
class Api(object):
  ...
  def GetFriendsTimeline(self,
    ...
    '''
    if user:
      url = 'https://twitter.com/statuses/friends_timeline/%s.json' % user
  ...
  def GetFavorites(self):
    '''Fetch the sequence of twitter.Status messages that the user has
  ...
  def UpdateProfile(self, name=None, bio=None, location=None):
    '''Change information about a user's profile

    The twitter.Api instance must be authenticated

    Args:
      name:
        The user's new real name  [Optional]
      bio: 
        The user's new bio [Optional]
      location: 
        The user's new location [Optional]
    '''
    if not name and not bio and not location:
	raise TwitterError("Must supply at least one argument")
    if not self._username:
	raise TwitterError("Must be authenticated")
    parameters = {}
    if name:
    ...
    json = self._FetchUrl(url, parameters=parameters, post_data=parameters)
    data = simplejson.loads(json)
    self._CheckForTwitterError(data)
    return parameters

  def UpdateProfileBackgroundColor(self, color):
    '''Change the user's background color

    The twitter.Api instance must be authenticated

    Args:
      color: 
        The new color (in #RRGGBB format)
    '''
    if not color:
	raise TwitterError("Must supply a color")
    if not self._username:
	raise TwitterError("Must be authenticated")
    url = 'https://twitter.com/account/update_profile_colors.json'
    color = color.lstrip("#")
    parameters = {}
    parameters['profile_background_color'] = color
    json = self._FetchUrl(url, parameters=parameters, post_data=parameters)
    data = simplejson.loads(json)
    self._CheckForTwitterError(data)
    return color   

  def UpdateProfileSidebarColor(self, color):
    '''Change the user's sidebar color
    ...
    color = color.lstrip("#")
    parameters = {}
    parameters['profile_sidebar_fill_color'] = color
    json = self._FetchUrl(url, parameters=parameters, post_data=parameters)
    data = simplejson.loads(json)
    self._CheckForTwitterError(data)
    return color 
  ...
  def SearchTwitter(self, query):
    if not query:
	raise TwitterError("Must provide a search query")
    url = "http://search.twitter.com/search.json" + "?" + urllib.urlencode({"q": self.unicode2utf8(query)})
    json = self._FetchUrl(url,dont_add_source=1)
    data = simplejson.loads(json)

    results = data['results']  
    tweets = []
    for tweet in results:
      url2 = 'https://twitter.com/users/show/%s.json' % tweet['from_user']
      json2 = self._FetchUrl(url2)
      data2 = simplejson.loads(json2)
      user = User.NewFromJsonDict(data2)
      tweets.append(Status.NewFromJsonDict(tweet, user))

    return tweets

  def UpdateProfileTextColor(self, color):
    '''Change the user's text color
    ...
    color = color.lstrip("#")
    parameters = {}
    parameters['profile_text_color'] = color
    json = self._FetchUrl(url, parameters=parameters, post_data=parameters)
    data = simplejson.loads(json)
    self._CheckForTwitterError(data)
    return color  
  ...
  def GetUserTimeline(self, user=None, count=None, since=None, since_id=None):
    ...
    if since_id:
      parameters['since_id'] = since_id
    if user:
      url = 'https://twitter.com/statuses/user_timeline/%s.json' % user
    elif not user and not self._username:
      raise TwitterError("User must be specified if API is not authenticated.")
    else:
  ...
  def GetStatus(self, id):
    ...
    url = 'https://twitter.com/statuses/show/%s.json' % id
    json = self._FetchUrl(url)
    data = simplejson.loads(json)
    self._CheckForTwitterError(data)
  ...
  def DestroyStatus(self, id):
    ...
    url = 'https://twitter.com/statuses/destroy/%s.json' % id
  ...
  def GetFriends(self, user=None, page=None):
    ...
    if user:
      url = 'https://twitter.com/statuses/friends/%s.json' % user 
    else:
  ...
  def GetFeatured(self):
    ...
    '''
    url = 'https://twitter.com/statuses/featured.json'
    json = self._FetchUrl(url)
    data = simplejson.loads(json)
    self._CheckForTwitterError(data)
    return [User.NewFromJsonDict(x) for x in data]
  ...
  def GetUser(self, user):
    ...
    '''
    url = 'https://twitter.com/users/show/%s.json' % user
    json = self._FetchUrl(url)
    data = simplejson.loads(json)
    self._CheckForTwitterError(data)
  ...
  def DestroyDirectMessage(self, id):
    ...
    '''
    url = 'https://twitter.com/direct_messages/destroy/%s.json' % id
    json = self._FetchUrl(url, post_data={})
    data = simplejson.loads(json)
    self._CheckForTwitterError(data)
    return DirectMessage.NewFromJsonDict(data)
  ...
  def CreateFriendship(self, user):
    ...
    '''
    url = 'https://twitter.com/friendships/create/%s.json' % user
    json = self._FetchUrl(url, post_data={})
    data = simplejson.loads(json)
    self._CheckForTwitterError(data)
    return User.NewFromJsonDict(data)
  ...
  def DestroyFriendship(self, user):
    ...
    '''
    url = 'https://twitter.com/friendships/destroy/%s.json' % user
    json = self._FetchUrl(url, post_data={})
    data = simplejson.loads(json)
    self._CheckForTwitterError(data)
    return User.NewFromJsonDict(data)
  ...
  def CreateFavorite(self, status):
    ...
    if status.favorited:
	return
    url = 'https://twitter.com/favorites/create/%s.json' % status.id
    json = self._FetchUrl(url, post_data={})
    data = simplejson.loads(json)
    self._CheckForTwitterError(data)
    return Status.NewFromJsonDict(data)
  ...
  def DestroyFavorite(self, status):
	...
	return
    url = 'https://twitter.com/favorites/destroy/%s.json' % status.id
    json = self._FetchUrl(url, post_data={})
    data = simplejson.loads(json)
    self._CheckForTwitterError(data)
    return Status.NewFromJsonDict(data)
  ...
  def GetUserByEmail(self, email):
    ...
    Returns:
      A twitter.User instance representing that user
    '''
    url = 'https://twitter.com/users/show.json?email=%s' % email
    json = self._FetchUrl(url)
    data = simplejson.loads(json)
    self._CheckForTwitterError(data)
    return User.NewFromJsonDict(data)
  ...
  def _FetchUrl(self,
    ...
    Args:
      ...
      dont_add_source: If true, does not attach Source (for Search API)
